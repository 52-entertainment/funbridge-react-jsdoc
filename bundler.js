...
const VUE_WRAPPER = process.env.IS_DEV
  ? 'src/vue-wrapper.js'
  : 'lib/vue-wrapper.js';
const REACT_WRAPPER = process.env.IS_DEV
...
module.exports = function bundle(Components, out, config) {
  ...
  if (vueComponents.length) {
    init =
      ...
      import VueWrapper from '${pathCrossEnv(
        path.relative(absoluteOut, path.join(__dirname, VUE_WRAPPER))
      )}';\n
  ...
  if (config.betterDocs.component) {
    ...
    if (
      config.betterDocs.component.entry &&
