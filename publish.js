...
function generate(title, subtitle, docs, filename, resolveLinks) {
  ...
  if (subtitle === 'Components' && docData.docs[0].properties) {
    docData.docs[0].component.props = docData.docs[0].properties;
  }
  ...
  subCategoryNames.forEach((subCategoryName) => {
    ...
    if (subCategoryItems.length) {
      ...
      if (itemsNav !== '') {
        ...
        if (subCategoryName === '') {
          // Passer à true si on veut rajouter "globals"
          if (subCategoryNames.length === 1 && showSingleSubCategory) {
            globalsNav = '<ul>' + itemsNav + '</ul>';

            // globalsNav += '<ul>' + itemsNav + '</ul>'
          } else {
            globalsNav = '<h3>' + heading + '</h3><ul>' + itemsNav + '</ul>';
          }
        } else {
...
function buildGroupNav({ members, title, showSingleSubCategory = false }) {
  var globalNav;
  var seenTutorials = {};
  var nav = '';
  var seen = {};
  ...
  nav += buildMemberNav({
    showSingleSubCategory,
    items: members.components || [],
    itemHeading: 'Components',
    itemsSeen: seen,
    linktoFn: linkto,
  });
  nav += buildMemberNav({
...
function buildNav(members, navTypes = null, betterDocs) {
  ...
  var topNav = navTypes ? '' : `<h2><a href="${href}">Documentation</a></h2>`;
  var nav = '';
  ...
  types.forEach(function (type) {
    ...
    members[type].forEach(function (element) {
      ...
      if (element.category) {
        ...
        if (!categorised[element.category][type]) {
          categorised[element.category][type] = [];
        }
        categorised[element.category][type].push(element);
      } else {
        rootScope[type]
          ? rootScope[type].push(element)
          : (rootScope[type] = [element]);
      }
    });
  });
  ...
  Object.keys(categorised)
    .sort()
    .forEach((category) => {
      if (Object.keys(special).includes(category)) {
        special[category] = buildGroupNav({
          members: categorised[category],
          title: category,
          showSingleSubCategory: true,
        });
      } else {
        nav += buildGroupNav({
          members: categorised[category],
  ...
  Object.keys(special).forEach((category) => {
    if (special[category]) {
      topNav += special[category];
    }
  });

  const finalNav = (topNav += mainNav += nav);
...
exports.publish = function (taffyData, opts, tutorials) {
  ...
  if (opts.tutorials) {
    // sort tutorials
    try {
      const tutorialsFile = JSON.parse(
        fs.readFileSync(`${opts.tutorials}/tutorials.json`)
      );
      members.tutorials = Object.keys(tutorialsFile).map(
        (key) => tutorials._tutorials[key]
      );
  ...
  members.classes = helper.find(data, {
    kind: 'class',
    component: { isUndefined: true },
  });
